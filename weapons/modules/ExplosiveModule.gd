...
func on_collision(_collision: Dictionary, bullet: Bullet) -> void:
	# Use the bullet's position instead of the collision point
	var explosion_center: Vector3 = bullet.global_position
	var aoe_damage = bullet.damage * explosion_damage_multiplier
	spawn_explosion(explosion_center, bullet.get_tree())
	var enemies = bullet.get_tree().get_nodes_in_group("enemies")
	for enemy in enemies:
		if enemy.is_in_group("enemies") and enemy.has_method("take_damage"):
			if enemy.global_position.distance_to(explosion_center) <= explosion_radius:
